function [Population,worst,tensor] = Reduce(N,M,Population,W,worst,tensor,r,row)
...
%--------------------------------------------------------------------------  
    ...
    tensor = UpdateUtilityTensor(N,PopObj,worst,r,tensor,W,row);
    ...
    if N > 1 && N < N+1
        ...
        deltaS = CalR2HVC(M,tensor1);
    elseif N==1
        deltaS = 0;
    else
        deltaS = CalR2HVC(M,tensor);
    end
    
    %% Delete the worst solution from the last front
    [~,worst] = min(deltaS);
    %Population(LastFront(worst),:) = [];
    Population.X(LastFront(worst),:)=[];
    Population.objs(LastFront(worst),:) = [];
