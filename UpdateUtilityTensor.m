function tensor = UpdateUtilityTensor(N,PopObj,worst,r,tensor,W,row)
...
%-------------------------------------------------------------------------- 
    ...
    utilityPoints = zeros(row,N+1);
    lastPoints = zeros(row,N+1);
    for j = 1:row
        currWeights = repmat(W(j,:), N+1,1);
    ...
    utilityPoints = zeros(N,row);
    for j = 1:row
        currWeights = repmat(W(j,:), N, 1);
